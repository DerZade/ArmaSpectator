...
private _cam = [] call zade_spectator_fnc_camera;
private _drawVehicles = [];

//------------------------- draw normal units -------------------------
{
     private _distance = _x distance _cam;
     ...
     if (!(vehicle _x isEqualTo _x) and _distance > 300) then {
          ...
          _position set [2, (_position select 2) + 0.75];

          drawIcon3D [_icon, _color, _position, 0.75, 0.75, 0, ""];
...
} forEach (([] call zade_spectator_fnc_allUnits) select {_x distance _cam < 2000});

//------------------------- draw vehicles -------------------------
{
     private _icon = getText (configFile >> "CfgVehicles" >> (typeOf _x) >> "Icon");

     private _color = [side group ((crew _x) select 0)] call BIS_fnc_sideColor;
     private _alpha = if ((_x distance _cam) > 1500) then {(2000 - (_x distance _cam)) / 500} else {1};
     _color set [3,_alpha];

     private _position = getPos _x;
     _position set [2, (_position select 2) + 0.75];

     drawIcon3D [_icon, _color, _position, 0.5, 0.5, 0, ""];
} forEach _drawVehicles;
...
//------------------------- draw custom icons -------------------------
{
      ...
      private _distance = _pos distance _cam;
